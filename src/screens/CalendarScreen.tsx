...
export function CalendarScreen() {
  ...
  return <ScrollView style={s.screen} contentContainerStyle={s.content}>
    ...
    <View style={[s.card, { padding: 12 }]}>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>{monthDays(month).map(date => {
        const key = dayKey(date); const count = counts.get(key) ?? 0; const active = selected === key;
        return <Pressable key={key} accessibilityRole="button" accessibilityState={{ selected: active }} accessibilityLabel={`${date.getMonth() + 1}月${date.getDate()}日 締切${count}件`} onPress={() => setSelected(key)}
          style={{ width: '14.2857%', minHeight: 56, alignItems: 'center', justifyContent: 'center', gap: 3, borderRadius: 12, backgroundColor: active ? colors.green : key === dayKey(now) ? colors.pale : 'transparent' }}>
          ...
          <Text style={{ fontSize: 10, color: active ? '#E4F1C4' : colors.green, minHeight: 13 }}>{count ? `${count}件` : ''}</Text>
        </Pressable>;
      })}</View>
