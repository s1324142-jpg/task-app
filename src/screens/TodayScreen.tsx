...
import { courseNameForAssignment } from '../domain/assignments';
...
export function TodayScreen() {
  ...
  return <ScrollView style={s.screen} contentContainerStyle={s.content}>
    ...
    {shouldDoToday.length ? shouldDoToday.map(item => <AssignmentCard key={item.id} assignment={item} courseName={courseNameForAssignment(item, data?.courses ?? [])} now={now} reason={priorityReason(item, now)} onPress={() => nav.navigate('Detail', { id: item.id })} />)
    ...
    {flexible.length ? flexible.map(item => <AssignmentCard key={item.id} assignment={item} courseName={courseNameForAssignment(item, data?.courses ?? [])} now={now} reason={priorityReason(item, now)} onPress={() => nav.navigate('Detail', { id: item.id })} />)
