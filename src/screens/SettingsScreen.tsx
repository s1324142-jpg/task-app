...
export function SettingsScreen() {
  ...
  const openManabaLogin = async (mode: 'login' | 'sessionCheck') => {
    if (Platform.OS === 'web') {
  ...
  return <ScrollView style={s.screen} contentContainerStyle={s.content}>
    ...
    <View style={s.card}>
      ...
      <Text style={s.muted}>大学が案内しているURLを入力します。IDやパスワードは入力・保存しません。</Text>
      ...
      <Button disabled secondary title="課題を同期（HTML解析未設定）" onPress={() => undefined} />
      ...
      <Text style={s.muted}>{Platform.OS === 'web' ? 'manaba連携はAndroid / iOSアプリで利用できます。' : '同期時は保存済みWebViewセッションを確認します。課題HTML解析は対象大学の実データ確認後に有効になります。'}</Text>
    </View>
